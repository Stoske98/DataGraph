...
namespace DataGraph.Tests.Editor
{
    ...
    public class CodeGeneratorTests
    {
        ...
        public void DictionaryField_GeneratesDictionaryProperty()
        {
            ...
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(result.Value.Contains("public Dictionary<string, Prop> props"));
        }
        ...
        [Test]
        public void DoubleField_GeneratesDoubleType()
        {
            var graph = MakeGraph(new ParseableObjectRoot("Config", new ParseableNode[]
            {
                new ParseableCustomField("precision", "A", FieldValueType.Double),
            }));

            var result = _gen.GenerateEntries(graph);

            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(result.Value.Contains("public double precision;"));
        }

        [Test]
        public void EnumField_GeneratesEnumTypeName()
        {
            var graph = MakeGraph(new ParseableObjectRoot("Item", new ParseableNode[]
            {
                new ParseableEnumField("type", "A", "ItemType"),
            }));

            var result = _gen.GenerateEntries(graph);

            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(result.Value.Contains("public ItemType type;"));
        }

        [Test]
        public void FlagField_GeneratesFlagTypeName()
        {
            ...
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(result.Value.Contains("public StatusFlag status;"));
        }

        [Test]
        public void StringKey_DictionaryRoot_GeneratesStringKeyDatabase()
        {
            ...
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(result.Value.Contains("public class TestDatabase : DictionaryDatabaseAsset<string, Region>"));
        }

        [Test]
        public void DeeplyNested_ThreeLevels_GeneratesAllClasses()
        {
            var graph = MakeGraph(new ParseableDictionaryRoot("Item", "A", KeyType.Int, new ParseableNode[]
            {
                new ParseableObjectField("stats", "Stats", new ParseableNode[]
                {
                    new ParseableObjectField("combat", "CombatStats", new ParseableNode[]
                    {
            ...
            var code = result.Value;
            Assert.IsTrue(code.Contains("public class Item : DataGraphEntry"));
            Assert.IsTrue(code.Contains("public class Stats"));
            Assert.IsTrue(code.Contains("public class CombatStats"));
            Assert.IsTrue(code.Contains("public int damage;"));
        }

        [Test]
        public void DictionaryField_PrimitiveSingleValue_InlinesType()
        {
            var graph = MakeGraph(new ParseableDictionaryRoot("Item", "A", KeyType.Int, new ParseableNode[]
            {
                new ParseableDictionaryField("bonuses", null, "C", KeyType.String, new ParseableNode[]
                {
                    new ParseableCustomField("value", "D", FieldValueType.Int),
                }),
            }));
            ...
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(result.Value.Contains("public Dictionary<string, int> bonuses"));
        }
